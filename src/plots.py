...
def aggregate_node_removal_results(
    results: pd.DataFrame
) -> pd.DataFrame:
    """
    Aggrega i risultati della rimozione nodi
    calcolando media e deviazione standard
    delle repliche.
    """
    ...
    aggregated = (
        ...
        .agg(
            ...
            mean_surviving_seed_count=(
                "surviving_seed_count",
                "mean"
            ),
            std_surviving_seed_count=(
                "surviving_seed_count",
                "std"
            ),
            baseline_influenced_nodes=(
...
def plot_node_removal_composite(
    aggregated_results: pd.DataFrame,
    cost_function: str,
    output_path: str,
    total_nodes: int
    ...
    X = percentuale di nodi rimossi (incluso 0%)
    Y = |Inf[G',S']|
    """
    ...
    fig.suptitle(
        f"Node Removal Robustness - {cost_function} Costs",
        fontsize=14
    )
